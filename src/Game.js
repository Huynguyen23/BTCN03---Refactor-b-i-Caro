...
class Game extends React.Component {
    ...
    handleClick(i) {
        ...
        for (let i = tmp; i <= this.state.stepNumber; i++) {
        ...
        if (this.state.reversed) {
            history.splice(1,0,{value:value});
            this.setState({
                history: history,
                //squares: squares,
    ...
    render() {
        ...
        for (let i = tmp; i <= this.state.stepNumber; i++) {
            ...
            if(this.state.history[i].value[0]!==null){
        ...
        if (this.state.reversed) {
            moves = history.map((step, move) => {
                const backgroundBtn = (move === (this.state.history.length-this.state.stepNumber) && this.state.stepNumber !== 0) ?
                    "btn-chosen" :
                    "btn-play";
                const desc = move ?
                    'Go to move ' + (this.state.history.length - move) + " (" + (Math.floor(step.value[0] / 20)) + ':' + (step.value[0] % 20) + ')' :
                    'Go to game start';
                return (
                    <li key={move}>
            ...
            moves = history.map((step, move1) => {
                const backgroundBtn = (move1 === this.state.stepNumber && this.state.stepNumber !== 0) ?
                    "btn-chosen" :
                    "btn-play";
                const desc = move1 ?
                ...
                return (
                    <li key={move1}>
                        <button className={backgroundBtn} onClick={() => this.jumpTo(move1)}>{desc}</button>
                    </li>
                )
            });
        }
